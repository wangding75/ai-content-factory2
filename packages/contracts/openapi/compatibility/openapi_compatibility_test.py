...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--self-test':
        destructive = json.loads(json.dumps(current_inventory))
        victim = destructive['paths'][0]
        destructive['paths'] = destructive['paths'][1:]
        errors = check_compatibility(current_inventory, destructive)
        expected = f"Deleted path/method: {victim['method'].upper()} {victim['path']}"
        if expected not in errors:
            print(f"[FAIL] Compatibility self-test did not detect: {expected}")
            sys.exit(1)
        schema_name = next(name for name, schema in destructive['schemas'].items() if schema.get('properties'))
        field_name = next(iter(destructive['schemas'][schema_name]['properties']))
        del destructive['schemas'][schema_name]['properties'][field_name]
        errors = check_compatibility(current_inventory, destructive)
        expected = f"Deleted field '{field_name}' in schema '{schema_name}'"
        if expected not in errors:
            print(f"[FAIL] Compatibility self-test did not detect: {expected}")
            sys.exit(1)
        print("[PASS] OpenAPI compatibility self-test rejected destructive path and field samples.")
        sys.exit(0)
