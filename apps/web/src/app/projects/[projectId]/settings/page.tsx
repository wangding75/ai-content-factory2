...
export default async function ProjectSettingsPage({ params, searchParams }: { params: Promise<{ projectId: string }>; searchParams: Promise<{ tab?: string }> }) {
  ...
  return <AppShell active="projects"><ProjectWorkspaceFrame project={project} active="settings"><main className="project-settings-page"><nav className="project-settings-tabs" aria-label="项目设置"><Link className={!workflowTab ? "active" : ""} href={`/projects/${projectId}/settings`}>基本信息</Link><Link className={workflowTab ? "active" : ""} href={`/projects/${projectId}/settings?tab=workflow-bindings`}>工作流绑定</Link><button type="button" disabled>其他设置</button></nav>{workflowTab ? <WorkflowBindingsPage projectId={projectId} /> : <section className="project-settings-placeholder"><h2>基本信息</h2><p>项目基础信息将在后续版本开放编辑。</p><div className="project-settings-entry-card"><div><h3>工作流绑定配置</h3><p>为章节规划、内容生成、审核和改写配置工作流</p></div><Link className="project-settings-entry-btn" href={`/projects/${projectId}/settings?tab=workflow-bindings`}>前往工作流绑定</Link></div><Link className="project-settings-back" href={`/projects/${projectId}`}>返回项目概览</Link></section>}</main></ProjectWorkspaceFrame></AppShell>;
}
