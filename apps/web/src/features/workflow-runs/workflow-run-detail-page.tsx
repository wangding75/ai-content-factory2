...
function Confirm({ action, busy, retryMode, retryOptions, onCancel, onConfirm, onRetryModeChange }: { action: RunAction; busy: boolean; retryMode: WorkflowRunRetryMode; retryOptions: WorkflowRunRetryOptions | null; onCancel: () => void; onConfirm: () => void; onRetryModeChange: (mode: WorkflowRunRetryMode) => void }) {
...
export function WorkflowRunDetailPage({ runId }: { runId: string }) {
  ...
  if (isFailure) return <><FailureRunDetail run={run} events={events} eventsError={eventsError} retryOptions={retryOptions} retryAvailable={retryAvailable} notice={notice} onAction={setAction} onRetryEvents={() => void listWorkflowRunEvents(runId).then(setEvents).then(() => setEventsError(false)).catch(() => setEventsError(true))} />{action && <Confirm action={action} busy={busy} retryMode={retryMode} retryOptions={retryOptions} onCancel={() => setAction(null)} onConfirm={() => void doAction()} onRetryModeChange={setRetryMode} />}</>;
