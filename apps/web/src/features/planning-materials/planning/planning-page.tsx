...
const isPlanningCompleted = (data: ProjectPlanning) => data.version > 0 && Boolean(data.premise.trim() || data.audience.trim() || data.style.trim() || data.goals_json.plot_summary.trim() || data.goals_json.selling_points.length || data.constraints_json.emotional_tone.trim());
...
export function PlanningPage({ projectId }: { projectId: string }) {
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    try { const [nextProject, planning] = await Promise.all([getProjectWorkspace(projectId).then(({ project }) => project), getProjectPlanningFromApi(projectId, { signal })]); setProject(nextProject); setSaved(planning); setForm(toForm(planning)); setEditing(!isPlanningCompleted(planning)); }
  ...
  const save = async () => { if (!saved || saving || !isEditing) return; if (tagText.trim()) { addTag(); } const nextErrors = validate(form); setErrors(nextErrors); if (Object.values(nextErrors).some(Boolean)) return; setSaving(true); setSaveError(null); setNotice(null); try { const result = await saveProjectPlanningToApi(projectId, { ...form, expected_version: saved.version }); setSaved(result); setForm(toForm(result)); setEditing(false); setTagText(""); setNotice("策划方案已保存。"); } catch (error) { const apiError = error instanceof ApiError ? error : null; setSaveError(apiError?.code === "VERSION_CONFLICT" ? "当前数据已被更新，未覆盖你的输入。请重新加载后再保存。" : "保存失败，已保留当前输入。请稍后重试。"); } finally { setSaving(false); } };
  const cancel = () => { if (saved) { setForm(toForm(saved)); setEditing(!isPlanningCompleted(saved)); setErrors({}); setSaveError(null); setNotice(null); } };
  ...
  return <div className="planning-workspace"><header className="planning-project-header"><div className="planning-header-main"><div className="planning-breadcrumb"><Link href="/projects">项目管理</Link><Icon name="arrowRight" size={14}/><span>{project.name}</span></div><div className="planning-project-title"><h1>{project.name}</h1><span>{planningStatus}</span><i>S02</i></div></div></header><ProjectWorkspaceNav projectId={projectId} active="planning" />
    <main className="planning-main"><section className="planning-content"><div className="planning-intro"><h2>项目策划</h2><p>明确创作方向，让后续的素材与故事创作保持一致。</p></div>{dirty && <div className="planning-dirty"><span><Icon name="info" size={17}/>有未保存的更改</span><button onClick={save} disabled={saving || !isEditing}>立即保存</button></div>}{notice && <div className="planning-notice success">{notice}</div>}{saveError && <div className="planning-notice error" role="alert"><span>{saveError}</span>{saveError.includes("重新加载") && <button onClick={() => void load()}>重新加载</button>}</div>}
      <section className="planning-card"><h3><Icon name="chart" size={20}/>项目定位</h3><div className="planning-grid"><Field label="核心主题" error={errors.premise}><input disabled={!isEditing} value={form.premise} onChange={(event) => update("premise", event.target.value)} maxLength={500}/></Field><Field label="目标受众" error={errors.audience}><input disabled={!isEditing} value={form.audience} onChange={(event) => update("audience", event.target.value)} maxLength={500} placeholder="例如：20-35 岁硬核科幻爱好者"/></Field></div><Field label="核心卖点 (Selling Points)" error={errors.selling_points}><div className="tag-editor">{form.goals_json.selling_points.map((tag) => <span className="planning-tag" key={tag}>{tag}<button aria-label={`删除 ${tag}`} onClick={() => setForm((current) => ({ ...current, goals_json: { ...current.goals_json, selling_points: current.goals_json.selling_points.filter((value) => value !== tag) } }))}><Icon name="close" size={14}/></button></span>)}<div className="tag-add"><input disabled={!isEditing} value={tagText} maxLength={100} onChange={(event) => setTagText(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addTag(); } }} placeholder="添加卖点"/><button type="button" onClick={addTag}><Icon name="plus" size={15}/>添加标签</button></div></div></Field></section>
      <section className="planning-card"><h3><Icon name="edit" size={20}/>创作方向</h3><div className="planning-grid"><Field label="文学风格" error={errors.style}><input disabled={!isEditing} value={form.style} onChange={(event) => update("style", event.target.value)} maxLength={120} placeholder="例如：紧张、克制"/></Field><Field label="情感基调" error={errors.emotional_tone}><input disabled={!isEditing} value={form.constraints_json.emotional_tone} onChange={(event) => setForm((current) => ({ ...current, constraints_json: { emotional_tone: event.target.value } }))} maxLength={500}/></Field></div><Field label="核心剧情描述" error={errors.plot_summary}><textarea disabled={!isEditing} value={form.goals_json.plot_summary} onChange={(event) => setForm((current) => ({ ...current, goals_json: { ...current.goals_json, plot_summary: event.target.value } }))} maxLength={10000} rows={5} placeholder="描述故事的起始冲突与世界观核心机制…"/></Field></section>
      <section className="planning-readonly"><h3><Icon name="info" size={19}/>基础信息 <small>READ ONLY</small></h3><div><p><span>项目名称</span>{project.name}</p><p><span>创作类型</span>长篇小说</p><p><span>当前状态</span><b>{planningStatus}</b></p></div></section><footer className="planning-actions">{isEditing ? <><button className="secondary" onClick={cancel} disabled={saving}>取消</button><button className="primary" onClick={save} disabled={!dirty || saving}>{saving ? "保存中…" : planningCompleted ? "保存修改" : "保存策划方案"}</button></> : <button className="primary" onClick={() => setEditing(true)}>编辑策划方案</button>}</footer></section>
    <aside className="planning-preview"><p>项目摘要预览</p><article><div className="planning-cover"><Icon name="book" size={42}/><div><h3>{project.name}</h3><span>策划编号：P-2026-003</span></div></div><div className="planning-preview-body"><label>定位摘要</label><h2>{form.premise || "尚未填写核心主题"}</h2><p>{form.audience || "填写目标受众后，这里会显示项目定位摘要。"}</p><dl><div><dt>风格调性</dt><dd>{form.style || "未填写"}</dd></div><div><dt>创作状态</dt><dd>{planningStatus}</dd></div></dl><blockquote>{form.constraints_json.emotional_tone ? `“${form.constraints_json.emotional_tone}”` : "“等待定义项目的情感基调。”"}</blockquote><small>版本 {saved.version}{saved.updated_at ? ` · 最后修改：${new Date(saved.updated_at).toLocaleString("zh-CN")}` : " · 尚未保存"}</small></div></article></aside></main></div>;
}
