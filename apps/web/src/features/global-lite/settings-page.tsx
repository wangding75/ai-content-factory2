...
function ProviderDrawer({ drawer, types, onClose, onSaved }: { drawer: Exclude<Drawer, null>; types: LlmProviderTypeDto[]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [form, setForm] = useState(initial); const [error, setError] = useState<string>(); const [saving, setSaving] = useState(false); const [checking, setChecking] = useState(false); const [conflict, setConflict] = useState(false); const secretSupported = types.find(type => type.providerType === form.providerType)?.supportsSecret ?? true;
  ...
  const command = async (kind: "verify" | "discover" | "toggle") => { if (!editing || checking) return; setChecking(true); setError(undefined); try { const id = drawer.provider.id, version = drawer.provider.version, key = crypto.randomUUID(); if (kind === "verify") await verifyLlmProvider(id, version, key); else if (kind === "discover") await discoverLlmProviderModels(id, version, key); else await setLlmProviderEnabled(id, version, !drawer.provider.enabled, key); await onSaved(); setError(kind === "verify" ? "验证已提交，请查看最新验证状态。" : "操作已完成，已刷新最新状态。"); } catch (cause) { const api = cause instanceof ApiError ? cause : undefined; setConflict(api?.status === 409 || api?.code === "version_conflict"); setError(api?.status === 409 ? "配置已变化，请重新加载后再操作。" : "操作未完成，请稍后重试。"); } finally { setChecking(false); } };
