...
export async function listGlobalWorkflowRuns(query: { limit: number; offset: number }, init?: ApiRequestInit): Promise<Page<WorkflowRunVm>> { const response = await apiRequest<Page<GlobalWorkflowRunDto>>(`/content-workflow-runs${pageQuery(query)}`, init); return { ...response, items: response.items.map(mapWorkflowRun) }; }
...
export const globalLiteRequestPaths = { materials: "/api/v1/materials?scope=global", works: "/api/v1/works?scope=global", builtinWorkflows: "/api/v1/workflows/builtin", workflowRuns: "/api/v1/content-workflow-runs", capabilities: "/api/v1/capabilities", integrations: "/api/v1/integrations" } as const;
