...
export function RewriteResultPanel({ projectId, workId, runId, onRefresh }: Props) {
  ...
  return <main className="rewrite-page"><section className="rewrite-layout"><article><header><h1>重写候选版本</h1><p>{result.output.summary}</p></header>
  ...
  </article><article><h2>来源正文</h2><p>{source ?? "正在读取来源正文。"}</p><h2>候选正文</h2><p>{result.candidateVersion.content}</p></article></section>
  {confirming && <section role="dialog" aria-modal="true" aria-label="设为当前版本确认"><h2>设为当前版本</h2><p>确认将 Candidate v{result.candidateVersion.version_no} 设为当前版本。</p><dl><div><dt>当前版本</dt><dd>v{item.current_version.version_no}</dd></div><div><dt>目标 Candidate</dt><dd>v{result.candidateVersion.version_no}</dd></div><div><dt>固定来源版本</dt><dd>v{result.sourceContentVersionSummary.versionNo}</dd></div></dl><p>此操作只会切换 currentVersion，不会删除旧版本，也不会修改 ReviewReport 或 Issue。</p><p>若他人已切换当前版本，系统会拒绝本次提交并要求重新确认。</p><button type="button" disabled={submitting} onClick={() => setConfirming(false)}>取消</button><button type="button" className="primary" disabled={submitting} onClick={() => void setCurrent()}>{submitting ? "正在设为当前版本…" : "确认设为当前版本"}</button></section>}</main>;
}
...
function ResultState({ title, retry }: { title: string; retry?: () => void }) { return <main className="rewrite-page"><section className="rewrite-state"><h1>{title}</h1>{retry && <button type="button" onClick={retry}>重试</button>}</section></main>; }
