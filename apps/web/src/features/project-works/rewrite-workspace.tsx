...
function RewriteRunState({ projectId, workId, summary, run, events, retrying, cancelling, confirmCancel, onCancel, onDismissCancel, onConfirmCancel, onRetryRuntime, onRetryConsumption, onRefresh }: { projectId: string; workId: string; summary: RewriteSummary; run: WorkflowRunDto | null; events: WorkflowRunEventVm[]; retrying: boolean; cancelling: boolean; confirmCancel: boolean; onCancel: () => void; onDismissCancel: () => void; onConfirmCancel: () => void; onRetryRuntime: () => void; onRetryConsumption: () => void; onRefresh: () => void }) {
  ...
  return <main className="rewrite-page"><section className="rewrite-state"><h1>{failed ? (isConsumption ? "重写结果提交失败" : isValidation ? "重写输出未通过校验" : summary.state === "runtime_failed" ? "重写任务执行失败" : "重写任务已取消") : summary.state === "queued" ? "重写任务正在排队" : "重写任务正在运行"}</h1><p>{failed ? summary.latestError?.message ?? "任务未能完成，请按提示继续操作。" : summary.state === "queued" ? "任务已创建，正在等待执行。" : "正在依据固定来源版本生成候选正文。"}</p><dl><div><dt>固定来源版本</dt><dd>V{summary.sourceContentVersionSummary.versionNo} · {summary.sourceContentVersionSummary.title}</dd></div><div><dt>审核报告</dt><dd>已固定</dd></div><div><dt>已选问题</dt><dd>{summary.selectedIssueSummary?.total ?? 0} 个</dd></div><div><dt>当前运行</dt><dd>{run?.runNumber ?? "正在恢复"}</dd></div></dl>{events.length > 0 && <section><h2>执行进度</h2><ol>{events.map(event => <li key={event.id}>{event.title} · {event.createdAtLabel}</li>)}</ol></section>} {active(summary.state) && run?.status !== "cancelled" && <button type="button" disabled={cancelling} onClick={onCancel}>{cancelling ? "取消中…" : "取消运行"}</button>}{failed && !isConsumption && <button type="button" className="primary" disabled={!run || retrying} onClick={onRetryRuntime}>{retrying ? "重试中…" : "重新运行"}</button>}{isConsumption && <button type="button" className="primary" disabled={!run || retrying} onClick={onRetryConsumption}>{retrying ? "提交中…" : "重试提交结果"}</button>}<button type="button" onClick={onRefresh}>刷新状态</button>{confirmCancel && <section role="dialog" aria-modal="true"><h2>确认取消运行</h2><p>取消后该运行将停止，无法恢复为运行中。</p><button type="button" disabled={cancelling} onClick={onDismissCancel}>返回</button><button type="button" disabled={cancelling} onClick={onConfirmCancel}>{cancelling ? "取消中…" : "确认取消"}</button></section>}</section></main>;
}
