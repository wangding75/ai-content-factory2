...
export function ProjectsList() {
  ...
  useEffect(() => {
    ...
    void listProjects({ status: selected, limit: 100, offset: 0, signal: controller.signal }).then((result) => {
  ...
  return <>
    ...
    {state.kind === "success" && (state.items.length ? <div className="projects-grid"><CreateProjectCard />{state.items.map((project) => <ProjectCard key={project.id} project={project} />)}</div> : <EmptyState selectedLabel={selectedLabel} />)}
  </>;
}
...
function EmptyState({ selectedLabel }: { selectedLabel: string }) { return <section className="projects-empty"><span><Icon name="folder" size={28} /></span><h2>{selectedLabel === "全部" ? "还没有项目" : "暂无匹配项目"}</h2><p>{selectedLabel === "全部" ? "创建一个项目，开始内容创作。" : "可以切换筛选条件，或创建一个新项目。"}</p><Link href="/projects/new">新建项目</Link></section>; }
