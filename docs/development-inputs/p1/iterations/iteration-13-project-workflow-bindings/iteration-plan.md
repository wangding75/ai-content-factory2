...
- **当前阶段：Iteration 13 已完成并关闭（completed, acceptance: passed_with_user_waiver）。**
- **CF-13-05 人工 UI 验收已通过（基线 Commit: 2e8f3cd7e1a36c7d1e53d266666b2723f609ea6f）。**
- **CF-13-06 四环节真实 API 联调已通过。**
- **最终 E2E 测试跳过 2 项历史测试债务，经用户明确豁免，已关闭 Iteration 13。**
- **项目状态切换为 Iteration 14（next_iteration: 14）。**
...
详见冻结的 `data-model.md`。核心实体：`ProjectWorkflowBinding`。

## 6. API

冻结接口：

- `GET /projects/{projectId}/workflow-bindings` — 返回四个固定环节及绑定状态
- `PUT /projects/{projectId}/workflow-bindings/{stage}` — 创建或更换绑定
- `DELETE /projects/{projectId}/workflow-bindings/{stage}` — 解除绑定
- 复用 Iteration 12 全局工作流列表 API，增量增加可选 `applicableStage` 查询参数

精确请求体、响应体、错误码和并发字段详见 `api-scope.yaml` 和权威 OpenAPI 文件。
...
- [x] 刷新后绑定关系恢复；
- [x] 不发生跨项目数据串联；
- [x] PUT/DELETE 幂等与乐观锁通过；
- [x] 409 保留用户选择并支持加载最新配置；
- [x] 已停用、未接入和连接异常不会被误显示为"可执行"；
- [x] 不产生任何第三方出站请求；
- [x] 页面无重复工作区壳层；
- [x] 自动测试、真实数据库验证、人工 UI 验收和完整审核包通过（E2E 历史债务获用户豁免）。