...
- **当前阶段：数据模型与 API 契约已完整冻结（CF-13-02 + CF-13-02R）。**
- **CF-13-02R 契约修正已完成：补齐 401/403/解绑 404 错误码，DELETE expectedVersion 承载方式确定为 query parameter（复用现有 DELETE 惯例），新增 Iteration 13 Contract Test 门禁。**
- **OpenAPI、数据模型、错误结构、幂等、乐观锁、Audit 和并发契约已完整冻结。**
- **后端和前端可依据冻结契约独立开发。**
- **下一步：CF-13-03 后端 Migration 与 Domain 基础实现。**
- **不得在后续开发中自行改变字段、接口、错误码和 UI 语义。**
- **契约变更必须先回到契约阶段处理。**
