...
foreach($r in $refs){$src=Join-Path $RepoPath $r;if(Test-Path -LiteralPath $src){$dst=Join-Path $refdir $r; if((Get-Item $src).PSIsContainer){New-Item -ItemType Directory -Force $dst|Out-Null;Copy-Item -Path (Join-Path $src '*') -Destination $dst -Recurse -Force}else{New-Item -ItemType Directory -Force (Split-Path $dst)|Out-Null;Copy-Item -LiteralPath $src -Destination $dst -Force}}}
